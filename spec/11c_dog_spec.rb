...
describe Dog do
  ...
  subject(:digong) { described_class.new('digong', 'askal')}

  # Write a test using the second shared_example to test that dog responds to
  # talk ('WOOF!').
  context 'when Dog can respond to talk with WOOF!' do
    include_examples 'shared method name'
  end

  # remove the 'x' before running this test
  it 'is not barking' do
    expect(digong).not_to be_barking
  end

  # remove the 'x' before running this test
  it 'is sleeping' do
    expect(digong).to be_sleeping
  end
end
